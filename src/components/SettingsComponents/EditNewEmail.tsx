...
const translations:any = {
  "From Emails":"settings_sms_from-emails",
}

const EditNewEmail = React.memo(({ services }: IProps) => {
  ...
  const handleAdd = useCallback(async () => {
    if (token) {
      const [dataRes, dataErr] = await handle(
        Settings.postEmailService(token, {
          ...editData,
  ...
  return (
    <StyledWrapper>
      <ServiceTitle>{t("settings_email-title2")}</ServiceTitle>
      <StyledDropdown
        label={t("settings_email-new-label")}
        placeholder={t("settings_email-new-label")}
          ...
          if (f.slug === "from") {
            return (
              <StyledEmailsList
                key={f.slug}
                options={[]}
                placeholder={t(translations[f.name]) || f.name}
                isValidNewOption={emailValidator}
                label={t(translations[f.name]) || f.name}
                value={
                  editData[f.slug]
                    ? editData[f.slug].map((v: number) => ({
                      value: String(v),
      ...
      <StyledButton data-action={activityList["new-email-save"]} onClick={handleAdd} disabled={!selectedServiceId}>{t("settings_email-add")}</StyledButton>
    </StyledWrapper>
  );
});

export default EditNewEmail;
